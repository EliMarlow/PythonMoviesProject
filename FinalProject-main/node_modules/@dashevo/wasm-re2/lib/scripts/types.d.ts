...
declare type RegExp2 = {
    ...
    _getStringPtrFromMatrix(arrayPtr: Pointer, raw: number, column: number): Pointer;
    /** @function _getQtyOfMatchedGroups : returns number of matched groups. */
    _getQtyOfMatchedGroups(textP: Pointer, regexP: Pointer, flagP: Pointer): number;
    ...
    _replace(basicPtr: Pointer, fromPtr: Pointer, toPtr: Pointer, flagPtr: Pointer): Pointer;
};
declare type RE2Functions = {
    testRegex(text: string): boolean;
    execRegex(text: string): string[][] | null;
    replaceString({ string, rewrite }: ReplaceInput): string;
};
declare type ReplaceParams = 'string' | 'rewrite';
declare type ReplaceInput = {
